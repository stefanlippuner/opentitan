...
class usbdev_common_vseq extends usbdev_base_vseq;
  ...
  virtual function void add_csr_exclusions(string           csr_test_type,
    ...
    if (csr_test_type != "hw_reset") begin
      // intr_test CSR affects the uart_state csr. It is already tested in intr_test.
    ...
    // Reading rxfifo when the fifo is empty, will result in unknown data assertion errors.
    csr_excl.add_excl({scope, ".", "rxfifo"}, CsrExclCheck);
  endfunction
